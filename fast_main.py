...
def TraceRay(O, D, t_min, t_max, depth):
    ...
    N = P - np.array(closest_sphere.center)  # вычисление нормали сферы в точке пересечения
    N = N/length(N)
    local_color = np.array(closest_sphere.color)*ComputeLighting(P, N, -D, closest_sphere.specular)
